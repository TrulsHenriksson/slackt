import { GetPerson, GetFamilyFromChild, } from './typesnmethods.js';
/** Append value to a key's array if the key exists, otherwise insert [value]. */
function map_append(m, key, value) {
    if (m.has(key)) {
...
function possibly_same_person(target, source, target_parents, source_parents, target_person, source_person, check_parents = true) {
    // Check that these fields match (if they are not empty)
    if (target_person.dateBirth !== ""
        && source_person.dateBirth !== ""
        && target_person.dateBirth !== source_person.dateBirth)
        return false;
    if (target_person.dateDeath !== ""
        && source_person.dateDeath !== ""
        && target_person.dateDeath !== source_person.dateDeath)
        return false;
    if (target_person.nameLastMaiden !== ""
...
/**
  ...
  */
export function identify_people(target, source) {
    let pairs = new Map();
    let unsure_pairs = new Map();
    ...
    for (let target_person of target.people) {
        let possible_sources = (possible_source_ids
